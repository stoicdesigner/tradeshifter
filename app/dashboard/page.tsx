    ...
    <div style={{ background:'var(--fcs-void)', minHeight:'100vh', fontFamily:'Inter,system-ui,sans-serif' }}>
      ...
      <main style={{ maxWidth:'1100px', margin:'0 auto', padding:'36px 24px 80px' }}>
        <div style={{ marginBottom:'28px' }}>
          <h1 style={{ fontSize:'24px', fontWeight:700, color:'var(--fcs-output)', marginBottom:'4px' }}>
            Welcome back{profile?.full_name ? `, ${(profile.full_name as string).split(' ')[0]}` : ''}
          </h1>
            ...
            <div style={{ position:'relative', width:'80px', height:'80px', flexShrink:0 }}>
              <svg width="80" height="80" viewBox="0 0 80 80" aria-label={`Export readiness score: ${score} out of 100`}>
                ...
                <circle cx="40" cy="40" r="32" fill="none" stroke={scoreColor} strokeWidth="8"
                  ...
                  style={{ transformOrigin:'center', transform:'rotate(-90deg) scaleX(-1)' }}/>
              </svg>
        ...
        <div className="card-depth" style={{ marginBottom:'16px' }}>
          ...
          {STEPS.map((s, i) => (
            <div key={s.label} style={{ display:'flex', alignItems:'center', gap:'14px', padding:'13px 0', borderBottom:i<STEPS.length-1?'1px solid var(--fcs-output-ghost)':'none' }}>
              <div style={{ width:'32px', height:'32px', borderRadius:'50%', background:s.done?'var(--fcs-signal)':'rgba(245,243,238,0.08)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, border:s.done?'none':'1px solid var(--fcs-output-ghost)' }}>
                {s.done
                  ? <svg width="14" height="11" viewBox="0 0 14 11" fill="none"><path d="M1 5l4 4 8-8" stroke="#0a0a0a" strokeWidth="2.5" strokeLinecap="round"/></svg>
                  : <s.icon size={15} color="var(--fcs-output-dim)"/>}
              </div>
              <p style={{ flex:1, fontSize:'13px', fontWeight:500, color:s.done?'var(--fcs-output-dim)':'var(--fcs-output)' }}>{s.label}</p>
              {!s.done && <Link href={s.href} style={{ textDecoration:'none' }}><button className="btn-ghost" style={{ fontSize:'12px', padding:'6px 12px' }}>Start</button></Link>}
        ...
        <div className="card-void">
          <div style={{ display:'flex', gap:'8px', alignItems:'flex-start' }}>
            <Shield size={14} color="var(--fcs-signal)" style={{ flexShrink:0, marginTop:'2px' }}/>
            <div>
              <p style={{ fontSize:'13px', fontWeight:500, color:'var(--fcs-output)', marginBottom:'4px' }}>Bangladesh Bank compliance reference</p>
              <Link href="/compliance" style={{ fontSize:'12px', color:'var(--fcs-signal)', textDecoration:'none', display:'inline-flex', alignItems:'center', gap:'4px', marginTop:'4px' }}>
                View FE Circulars 42, 43, 48 <ArrowRight size={12}/>
              </Link>
            </div>
          </div>
        </div>
